...
def global_evaluate(pf,cov,ax,ay,gridsize,grid_extent,k_mu=1.0, k_cov=1.0, k_range = 0.1):
    ...
    for gg in range(gridsize**2):
        ...
        scores.append(ps+us-range_penalty) # potential uncertainty reduction

    index = np.argmax(scores)
    return px[index],py[index],px,py,scores[index],uncertainty_scores[index],prediction_scores[index],scores, range_scores[index]
...
for t in range(mission_time):
    ...
    if auv.state:
        ...
        plt.savefig("fig/example4/update_step" + str(int(figno)) + ".png",dpi = 300)
        plt.close()
        figno += 1
