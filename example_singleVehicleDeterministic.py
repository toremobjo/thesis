...
fig.show()
fig.write_image("fig/DeterministicSinglevehicleExample/auv_paths.png")
...
fig.show()
fig.write_image("fig/DeterministicSinglevehicleExample/predictions.png")
...
fig.show()
fig.write_image("fig/DeterministicSinglevehicleExample/variance.png")

auv = auvs[0]
pfs = []
ts = []
for t in np.linspace(auv.df["t"].values[0],auv.df["t"].values[-1],num=20):
    if t > 1800:
        df = auv.df[auv.df["t"] < t]
        pfs.append(auv.gp.evaluate(df))
        ts.append(t)
...
for i in range(len(pfs)):
    ...
    for j, g in enumerate(grid):
        sim.append((getValue(g[0], g[1], g[2], time=ts[i], noise=False) - pfs[i][0][j]) ** 2)
    rmse.append(np.sqrt(np.mean(sim)))

plt.plot(ts,rmse)

grid = np.array(grid)
plt.savefig("fig/DeterministicSinglevehicleExample/rmse.png",dpi = 300)
plt.close()
