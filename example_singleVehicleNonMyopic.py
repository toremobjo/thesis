...
for run in range(run_iterations):
    for i, ag in enumerate(auvs):
        ## AUV STATE: 0=idle, 1=running
        if not ag.get_state():
            ag.wp_update_iteration += 1
            print(int(100*ag.update_iteration/run_iterations), "% please wait.")
            ...
            # End of prior, transition to adaptive phase
            elif ag.prior and ag.wp_update_iteration == len(ag.prior_wps):
                ag.prior = False
                ag.adapting = True
                ag.segment()
                ...
                #Exchange data with other AUVs
                for j in range(nv):
                    if auvs[j] != ag:
                        auvs[j].ingest_df(df)
                        auvs[j].ingest_pos(ag.get_xyz(),ag.vehicle_no)
                ag.set_wp(ag.adapt())
            #adaptive phase to the end
            elif ag.adapting:
                ag.segment()
                df = ag.df
                #Exchange data with other AUVs
