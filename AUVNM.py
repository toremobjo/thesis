...
class AUV:
    ...
    def generatePaths(self):
        ...
        for t in theta:
            for zz in dz:
                for zzz in dz:
                    paths.append(np.array([[r*np.cos(t),r*np.sin(t),zz],[2*r*np.cos(t),2*r*np.sin(t),zz+zzz]]))
                ...
                if path[0] > 0.0 and path[0] < self.grid_extent[0]:
                    if path[1] > 0.0 and path[1] < self.grid_extent[1]:
                        if path[2] > 0.0 and path[2] < self.grid_extent[2]:
                            ...
                            lmpp += pf[c]
                            luncc += pc[c,c]*3
                            lavoidd += avoidance_score
                            score += 3*ss+pf[c]+avoidance_score
                            #scores.append(3*ss+pf[c]+avoidance_score)
                            #wps.append([path[0], path[1], path[2]])
            scores.append(score)
            lmp.append(lmpp)
            lunc.append(luncc)
            lavoid.append(lavoidd)
            wps.append([pp[jj][0][0], pp[jj][0][1], pp[jj][0][2]])
